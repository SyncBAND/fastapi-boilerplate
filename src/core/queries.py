...
from src.core.database import Base, Propagation, Transactional
...
class BaseQuery(Generic[ModelType]):
    """Combined base class for data repositories and controllers."""
    ...
    async def get_by_id(self, id_: UUID, join_: set[str] | None = None) -> ModelType:
        ...
        """
        try:
            db_obj = await self.get_by(field="id", value=id_, join_=join_, unique=True)
            if not db_obj:
                raise NoResultFound
    ...
    async def get_by_uuid(self, uuid: UUID, join_: set[str] | None = None) -> ModelType:
        ...
        """
        try:
            db_obj = await self.get_by(field="uuid", value=uuid, join_=join_, unique=True)
            if not db_obj:
                raise NoResultFound
        except (NoResultFound, StatementError) as e:
            raise NotFoundException(
    ...
    async def get_by(
        ...
        :param join_: The joins to make.
        :param unique: Whether to return a unique result.
        :return: The model instance.
        """
        query = self._query(join_)
    ...
    async def _count(self, query: Select) -> int:
        ...
        result = await self.session.scalars(select(func.count()).select_from(query))
        return result.one()
